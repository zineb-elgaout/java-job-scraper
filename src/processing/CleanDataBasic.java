...
public class CleanDataBasic {
    ...
    private static void cleanData(Connection conn) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            // Créer la table nettoyée avec correction pour les apostrophes
            String createTable = """
                    CREATE TABLE cleaned_jobs AS
                    SELECT
                        -- Nettoyer le titre (échapper les apostrophes)
                        LOWER(TRIM(REPLACE(title, \"'\", \"''\"))) as title_clean,

                        -- Nettoyer l'entreprise
                        CASE
                            WHEN company IS NULL OR TRIM(company) = '' THEN 'non_specifie'
                            ELSE LOWER(TRIM(REPLACE(company, \"'\", \"''\")))
                        END as company_clean,

                        -- Nettoyer la localisation
                        CASE
                            WHEN location IS NULL OR TRIM(location) = '' THEN 'non_specifie'
                            ELSE LOWER(TRIM(REPLACE(location, \"'\", \"''\")))
                        END as location_clean,

                        -- Nettoyer la description
                        LOWER(TRIM(REPLACE(description, \"'\", \"''\"))) as description_clean,
